...
@router.get("/print-orders", response_model=List[dict])
async def get_all_print_orders(admin: UserModel = Depends(require_admin)):
    cursor = db.db.print_orders.find({}).sort("created_at", pymongo.DESCENDING)
    orders = await cursor.to_list(length=100)
    enriched_orders = []
    for order in orders:
        user_info = await db.db.users.find_one({"_id": ObjectId(order["user_id"])})
        enriched_orders.append({
            "id": str(order["_id"]),
            "order_id": order.get("order_id", ""),
            ...
            "pdf_file": order.get("pdf_file", ""),
            "pages": order.get("pages", 1),
            "copies": order.get("copies", 1),
            "color_mode": order.get("color_mode", "bw"),
            "binding": order.get("binding", "none"),
            "price": order.get("price", 0.0),
...
@router.put("/print-orders/{id}/status")
async def update_print_order_status(id: str, payload: dict, admin: UserModel = Depends(require_admin)):
    new_status = payload.get("status")
    if not new_status:
        raise HTTPException(status_code=400, detail="Status required")
        
    order = await db.db.print_orders.find_one({"_id": ObjectId(id)})
    if not order:
        raise HTTPException(status_code=404, detail="Print order not found")

    await db.db.print_orders.update_one(
        {"_id": ObjectId(id)},
        {"$set": {"order_status": new_status, "updated_at": datetime.now(timezone.utc)}}
    )

    buyer_notif = NotificationModel(
        user_id=ObjectId(order["user_id"]),
        title=f"Print Order Status Updated",
        message=f"Your print order {order.get('order_id', '')} status has been updated to: {new_status}."
    )
    await db.db.notifications.insert_one(buyer_notif.model_dump(by_alias=True, exclude_none=True))

    return {"message": "Print order status updated"}
